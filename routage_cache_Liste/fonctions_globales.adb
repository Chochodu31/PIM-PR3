...
package body Fonctions_globales is
	...
	procedure Rien_Interface_Ici(Inter : in out Unbounded_String) is
	begin
		Inter := To_Unbounded_String("");
	end Rien_Interface_Ici;

	procedure Ajout_cache (Routage: in T_Liste; Cellule : in T_Liste; Adresse_IP : in T_Adresse_IP; Cache : in out T_Liste; politique : in Tab_Politique; Cache_Taille : in Integer; Masque : in out T_Adresse_IP) is
		Destination : T_Adresse_IP;
		Int : Unbounded_String;
	begin
		Int := Avoir_Int(Cellule);
	...
	function association_ad_des (Cache : in out T_Liste ; Tab_Routage : in T_Liste; Adresse_IP : in T_Adresse_IP; Politique : in Tab_Politique; Cache_Taille : in integer; Nb_defaut : in out Integer) return Unbounded_String is
		Association : Integer;
		Int : Unbounded_String;
		Cellule : T_Liste;
		Masque : T_Adresse_IP;
	begin
		Association := 0;
		Asso(Cache, Adresse_IP, Association, Int, Cellule, Masque);
		if Association = 0 then
			Nb_defaut := Nb_defaut + 1;
			Asso(Tab_routage, Adresse_IP, Association, Int, Cellule, Masque);
			--  Put("=== Ajout dans la table de routage");
			--  New_Line;
			--  Put("Association : ");
			--  Put(Association);
			--  New_Line;
			--  Put("Masque : ");
			--  Afficher_Ad_IP(Masque);
			--  New_Line;
			--  Put("Adresse IP  : ");
			--  Afficher_Ad_IP (Adresse_IP);
			--  New_Line;
			--  Put("Interface : ");
			--  Afficher_int_ici (Int);
			--  New_Line;
			Ajout_cache(Tab_routage, Cellule, Adresse_IP, Cache, politique, Cache_Taille, Masque);
			if Association = 0 then
				raise Adresse_IP_Introuvable_Error;
			else
				Null;
...
--     end associer_routeur;
	...
	procedure Traiter_les_paquets(Entree : in File_Type; Sortie : in out File_Type; Tab_routage : in T_Liste; Cache : in out T_Liste; Politique : in Tab_Politique; Cache_Taille : in Integer; Nb_demande : out Integer; Nb_defaut : out Integer) is
		Texte : Unbounded_String;
		Ligne : Integer;
		IP_cmd : Boolean;
		Adresse_IP : T_Adresse_IP;
		...
		begin 
			...
			while not End_Of_File (Entree) loop
				...
				if IP_cmd then
					...
					--  New_Line;
					--  Put("=== Nouvelle Ligne ===");
					Int := association_ad_des (Cache, Tab_Routage, Adresse_IP, Politique, Cache_Taille, Nb_defaut);
					New_Line;
					--  Put("=== Nb_defaut ===");
					--  Put(Nb_defaut);
